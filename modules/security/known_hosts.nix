...
let
  knownHosts = nickname: keys: pkgs.writeText "${nickname}-known_host_file" (toString keys);
...
in
{
  ...
  config.programs.ssh.knownHostsFiles =
    ...
    ++ lib.mapAttrsToList (
      hostname: node: knownHosts hostname (map (key: "${hostname} ${key}") node.config.hey.hostKeys)
    ) nodes;
}
